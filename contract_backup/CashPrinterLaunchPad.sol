...
contract PolkaBridgeLaunchPad is Ownable, ReentrancyGuard {
  ...
  function addMulWhitelist(address[] memory user, uint256 pid) public onlyOwner {
  ...
  function IsWhitelist(address user, uint256 pid) public view returns (bool) {
    ...
    {
      return (whitelist[pid][user].IsWhitelist && whitelist[pid][user].IsActived);
  ...
  function purchaseIDO(uint256 pid) public payable nonReentrant {
    ...
    whitelist[pid][msg.sender].TotalETHPurchase = whitelist[pid][msg.sender].TotalETHPurchase.add(ethAmount);
    ...
    if (pools[poolIndex].Type == 2) {
      ...
      if (stakeAmount < 1500 * 1e18) {
        require(
          whitelist[pid][msg.sender].TotalETHPurchase <= pools[poolIndex].MaxPurchaseTier1,
          "invalid maximum purchase for tier1"
        );
      } else if (stakeAmount >= 1500 * 1e18 && stakeAmount < 3000 * 1e18) {
        require(
          whitelist[pid][msg.sender].TotalETHPurchase <= pools[poolIndex].MaxPurchaseTier2,
          "invalid maximum purchase for tier2"
        );
      } else {
        require(
          whitelist[pid][msg.sender].TotalETHPurchase <= pools[poolIndex].MaxPurchaseTier3,
      ...
      //community round
      require(
        whitelist[pid][msg.sender].TotalETHPurchase <= pools[poolIndex].MaxPurchaseTier3 * 2,
        "invalid maximum contribute"
      );
    }

    uint256 tokenAmount = ethAmount.mul(pools[poolIndex].RatePerETH).div(1e18);
    ...
    pools[poolIndex].TotalSold = pools[poolIndex].TotalSold.add(tokenAmount);
  }
  ...
  function claimToken(uint256 pid) public nonReentrant {
    ...
    require(
      block.timestamp >= pools[poolIndex].End.add(pools[poolIndex].LockDuration),
  ...
  function getBalanceTokenByPoolId(uint256 pid) public view returns (uint256) {
    uint256 poolIndex = pid.sub(1);
  ...
  function getPoolSoldInfo(uint256 pid) public view returns (uint256, uint256) {
    uint256 poolIndex = pid.sub(1);
